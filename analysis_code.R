...
# Likelihood:
	for(k in 1:K){
	    ...
	    censor.index[k] ~ dinterval(deaths[k], 
	    					c(deaths_lower[k], deaths_upper[k]))
		...
		cloglog_ifr[k] ~ dnorm(theta + theta1*Z[k], inv.var_tau);
  }
  
# Summary
g_IFR_9p = theta + theta1*(-1.041098);  
...
modelparams1<-c("theta", "theta1", "tau", "sigma")
...
data.frame(study_names="World Avg. (9% over 65yo)", rbind (c(world_wide))),
data.frame(study_names="USA Avg. (16% over 65yo)", rbind (c(usa_wide))),
data.frame(study_names="EU Avg. (20% over 65yo)", rbind (c(eu_wide)))
)
...
preds <- meta_IFR_plus[meta_IFR_plus [,"Study"] %in% 
				c("World Avg. (9% over 65yo)", 
                "USA Avg. (16% over 65yo)", 
                "EU Avg. (20% over 65yo)"), "IFR_labs"]

preds <- gsub("]", "}", preds, fixed=TRUE)
preds <- gsub("[", "{", preds, fixed=TRUE)

meta_IFR_plus[meta_IFR_plus [,"Study"] %in% c("World Avg. (9% over 65yo)", 
                "USA Avg. (16% over 65yo)", 
                "EU Avg. (20% over 65yo)"), "IFR_labs"] <- preds
...
data.frame(study_names="World Avg. (9% over 65yo)", rbind (c(world_wideIR))),
data.frame(study_names="USA Avg. (16% over 65yo)", rbind (c(usa_wideIR))),
data.frame(study_names="EU Avg. (20% over 65yo)", rbind (c(eu_wideIR)))
)
